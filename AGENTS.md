...
changes can crowd the interval out or exhaust the decode ceiling before the clip ends; it shells out to `ffmpeg-static`, `extractor.ts` is the only place that talks to OpenAI, `loop.ts`
