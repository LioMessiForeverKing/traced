...
to `ffmpeg-static` and throws `UnanalysableRecording` for the refusals a second read cannot change,
`extractor.ts` is the only place that talks to OpenAI, `loop.ts` claims work and owns the retry
story — every other failure retries, because reaching the bytes is the part that fails transiently. Nothing on the request path may call into it.
